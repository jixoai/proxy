...
我需要将配置文件中的 hooks 展示在界面中:

1. hook新增disabled属性
2. 界面上,提供的开关,可以实现对插件的启用和禁用
3. 注意,我们的内核是基于配置文件去计算hookId的,这个disabled需要被跳过不被计入
4. 编辑上支持自定义配置,目前type这个Select只有http这个选项.
5. 我们还支持快速配置“官方插件”,就是我们 packages 包中的`proxy-anthropic-ping`/`proxy-plugin-droid`, 提供一个Select来实现对字段的填充

---

插件系统新增一个功能: onResponse钩子可以获得request的元数据
注意: 包括url、headers、method,但是不包含body

其中一个用法是: 如果我的onRequest对请求做了重写, 我可以在headers中标记“私有字段”,我记得之前有这个提案, 我不知道是否完成了. 可以找一下相关的代码.然后我的response就可以读取到这个headers中的私有字段,来得某个插件是否被激活,然后再来决定是否对response进行重写. 否则目前只能得到reponse的特征, 很多时候无法满足判断需求.

完成这个功能后, 检查一下我们的插件,看哪个插件用得上这个功能. 先和我讨论你要怎么优化插件, 然后我再批准你去实现.

---

我需要在界面上实现“复制功能”,指对单条forward的复制,点击复制,会出现一个编辑Dialog,点击保存,会将这条的forward保存到配置文件中.

---

还是有问题，我只能看到 origin req/res body
另外Response Body中，如果proxy-plugin-openai4droid没有进行重写，它的 tab 应该是“disabled”的状态，否则如果能启用，那么就应该又被记录到数据库并且能被前端拿到。

同样的解决思路：先分析问题、写测试、复现问题、修复、测试通过

---

之前AI在某次重构的时候（支持流传输），破坏性更新了插件的协议。为了支持流，导致AI直接使用强制import 的方式，而不是我一开始预期的通过命令行启动某个进程，然后通过CALLBACK_URL来实现进场之间的握手通讯。

现在我需要回到非import的模式，而是继续使用进程启动的方式来启动插件。这就需要引入PRC的模式来为插件的流传输继续提供支持。

我的想法是这样：我们可能不需要RPC协议，而是使用全链路的http协议，我知道这会带来大量的转发，但这是符合预期的。也就是说，使用CONNECT协议来实现插件之间的互联。

这也就是说，插件既可以独立运行：启动后在终端打印一个proxy-url，也可以插入到我们的proxy系统中，我们自己去编排插件的hop-by-hop顺序

---

前端和数据库需要做配套的重构。如果还是依赖数据库来做整套系统，方案会太过复杂，而且可能难以调试。
我的建议是我们统一使用某种日志格式来存储的请求和响应的内容，这样不用担心数据库的问题，数据库只需要存储最原始的一些条目数据、索引数据，然后把= 主代理、插件代理的日志全部串起来。

这意味着我们需要重新设计我们的数据库结构、前端的请求列表和请求详情，都要重新设计。

---

我经常会遇到 `200 {"error":{"code":400,"message":"Upstream request failed","type":"server_error"} `这种“正确的异常”，遇到这种直接返回错误

`200 {"error":{"code":400,"message":"This request would exceed your account's rate limit. Please try again later.","type":"rate_limit_error"},"type":"error"}`
