...
export class PluginManager {
  ...
  private sortPlugins(plugins: BodyViewerPlugin[]): BodyViewerPlugin[] {
    ...
    plugins = plugins.filter(
      (p, i, self) => self.findLastIndex((p2) => p2.name === p.name) === i,
    );
    // 根据enforce排序
    ...
    const corePlugins = plugins.filter(
      (p) => null == p.enforce || p.enforce === "core",
    );
  ...
  addPlugin(plugin: BodyViewerPlugin): void {
    if (this.plugins.some((p) => p.name === plugin.name)) {
      console.warn(
        `[PluginManager] Plugin "${plugin.name}" already exists, skipping`,
      );
      return;
    }
    ...
    for (const plugin of this.plugins) {
      if (plugin.onInit) {
        try {
          ...
          console.error(
            `[PluginManager] Failed to initialize plugin "${plugin.name}":`,
            error,
          );
        }
      }
    ...
    for (const plugin of this.plugins) {
      if (plugin.onMount) {
        try {
          ...
          console.error(
            `[PluginManager] Failed to mount plugin "${plugin.name}":`,
            error,
          );
        }
      }
    ...
    for (const plugin of this.plugins) {
      if (plugin.onUnmount) {
        try {
          ...
          console.error(
            `[PluginManager] Failed to unmount plugin "${plugin.name}":`,
            error,
          );
        }
      }
  ...
  async executeTransformChain(
    ...
    for (const plugin of this.plugins) {
      ...
      try {
        ...
        console.error(
          `[PluginManager] Plugin "${plugin.name}" transform failed:`,
          error,
        );
        // 继续执行下一个插件
      }
  ...
  async notifyContentChange(
    ...
    for (const plugin of this.plugins) {
      ...
      try {
        ...
        console.error(
          `[PluginManager] Plugin "${plugin.name}" onContentChange failed:`,
          error,
        );
      }
    }
