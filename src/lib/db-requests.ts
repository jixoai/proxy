...
import type { BodyStage } from "./db-schema";
...
// 注意：ListSummary 在文件后面定义（viewer 列表页使用）
// InternalListSummary 用于内部查询结果
...
export interface RequestDetail {
  ...
  url: string;
  target_url: string | null;
  hooked_method: string | null;
  hooked_url: string | null;
  status: RequestStatus;
  error_message: string | null;
  abort_reason: AbortReason | null;
  client_aborted: boolean;
...
export function createRequest(input: CreateRequestInput): number {
  ...
  const stmt = db.query(`
    INSERT INTO requests (
      ...
      method, url, target_url, hooked_method, hooked_url, url_lc, path_lc,
  ...
  const result = stmt.run(
    ...
    input.hooked_method ?? null,
    input.hooked_url ?? null,
    url_lc,
    path_lc,
...
export function updateRequestHook(id: number, input: UpdateRequestHookInput): boolean {
  ...
  if (input.hooked_method || input.hooked_url) {
    db.query(`
      UPDATE requests
      SET hooked_method = COALESCE(?, hooked_method),
          hooked_url = COALESCE(?, hooked_url)
      WHERE id = ?
    `).run(input.hooked_method ?? null, input.hooked_url ?? null, id);
  }
...
export function getRequestDetail(id: number): RequestDetail | null {
  ...
  return {
    ...
    hooked_method: row.hooked_method ?? null,
    hooked_url: row.hooked_url ?? null,
    status: row.status,
    error_message: row.error_message,
    abort_reason: row.abort_reason,
    client_aborted: Boolean(row.client_aborted),
...
export function getProxyRequestById(id: number): LoggedRequest | null {
  ...
  return convertDetailToLoggedRequest(detail);
}
...
export function getRequestsAfterId(lastId: number): LoggedRequest[] {
  ...
  const results: LoggedRequest[] = [];
  for (const row of rows) {
    const detail = getRequestDetail(row.id);
    if (detail) {
      results.push(convertDetailToLoggedRequest(detail));
    }
  }
  return results;
}
...
function convertToListSummary(item: InternalListSummary): ListSummary {
  // 解析 plugin_info JSON
  let pluginInfo: PluginInfo | undefined;
  if (item.plugin_info) {
    try {
...
export function getAllRequestsSummary(
  ...
  return summaries.map(convertToListSummary);
}
...
function convertDetailToLoggedRequest(detail: RequestDetail): LoggedRequest {
  const result: LoggedRequest = {
    id: detail.id,
    ...
    request: {
      ...
      url: detail.url,
      targetUrl: detail.target_url ?? undefined,
  ...
  if (detail.has_request_hook_changes) {
    result.hookedRequest = {
      method: detail.hooked_method ?? detail.method,
      url: detail.hooked_url ?? detail.target_url ?? detail.url,
